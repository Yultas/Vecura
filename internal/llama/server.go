...
// Server manages a background llama-server process for local embeddings.
type Server struct {
	rm       *RuntimeManager
	cmd      *exec.Cmd
	pid      int
	...
	done     chan struct{} // closed when the managed process exits
}
...
func NewServer(rm *RuntimeManager) *Server {
	return &Server{
		rm:      rm,
		port:    8090, // default port for local embeddings
		baseDir: rm.BaseDir(),
	}
}
...
// Start launches llama-server in the background for text embeddings. If a
// server is already running (stale PID file or live process), it returns nil.
...
func (s *Server) isRunningLocked() bool {
	// For a process started by this manager, use cmd.Wait's completion
	// channel. Signal(0) is not a reliable liveness probe on Windows and can
	// report a live process as exited even while its HTTP server is responding.
	...
	// There may be a server left over from an earlier application instance.
	// Keep the PID-file fallback for that case.
	pid := s.loadPID()
	if pid == 0 {
		return false
	}
